...
class _InsightsScreenState extends State<InsightsViewBody> {
  ...
  final List<Map<String, dynamic>> memories = [
    {"date": DateTime.now(), "isFavorite": true, "hasImage": true, "mood": "Happy", "text": "Great day!"},
    {"date": DateTime.now().subtract(const Duration(days: 1)), "isFavorite": false, "hasImage": false, "mood": "Sad", "text": "Stressful"},
    {"date": DateTime.now().subtract(const Duration(days: 2)), "isFavorite": true, "hasImage": true, "mood": "Calm", "text": "Relaxing"},
     {"date": DateTime.now().subtract(const Duration(days: 2)), "isFavorite": true, "hasImage": true, "mood": "Calm", "text": "Relaxing"},
  ];
  ...
  Widget build(BuildContext context) {
    ...
    final favoritesCount = memories.where((m) => m["isFavorite"] == true).length;
    ...
    return ListView(
      ...
      children: [
        ...
        const Text('A look into your memories', style: TextStyle(color: Colors.grey)),
        const SizedBox(height: 24),

        // ✅ بطاقات الإحصائيات بشكل مبسط
        Row(
          children: [
            _buildExpandedStat('$totalCount', 'Memories', '📝', Colors.purple.shade50),
            const SizedBox(width: 10),
            _buildExpandedStat('$favoritesCount', 'Favorites', '❤️', Colors.pink.shade50),
            const SizedBox(width: 10),
            _buildExpandedStat('$withImagesCount', 'Photos', '📸', Colors.teal.shade50),
          ],
        ),
  ...
  Widget _buildExpandedStat(String value, String label, String emoji, Color bgColor) {
    return Expanded(
      child: _StatCard(value: value, label: label, emoji: emoji, color: bgColor),
    );
  }
}
...
class _MoodDistributionCard extends StatefulWidget {
  ...
  const _MoodDistributionCard({required this.distribution, required this.isDark});
...
class _MoodDistributionCardState extends State<_MoodDistributionCard> {
        ...
        children: [
          ...
          Row(
            children: [
              ...
              Expanded(
                child: Column(
                  children: sortedEntries.map((e) => _buildLegend(e.key, e.value, total)).toList(),
                ),
              ),
            ],
          ),
    ...
    return Padding(
      ...
      child: Row(
        children: [
          ...
          const Spacer(),
          Text('${(count / total * 100).round()}%', style: const TextStyle(fontSize: 12, color: Colors.grey)),
        ],
      ),
    );
  }
  ...
  Color _getColor(String mood) {
   switch (mood) {
      case 'Happy':      return const Color(0xFFFFD93D); // أصفر
      case 'Sad':        return const Color(0xFF6495ED); // أزرق هادي
      case 'Calm':       return const Color(0xFF98FB98); // أخضر نعناعي
      case 'Excited':    return const Color(0xFFFF6B6B); // أحمر مرجاني
      case 'Angry':      return const Color(0xFFC0392B); // أحمر داكن
      case 'Anxious':    return const Color(0xFF95A5A6); // رمادي
      case 'Grateful':   return const Color(0xFFF3A683); // برتقالي خوخي
      case 'Productive': return const Color(0xFF4BC0C0); // تركواز
      case 'Tired':      return const Color(0xFF574B90); // بنفسجي غامق
      case 'Loved':      return const Color(0xFFFF85A1); // وردي
      default:           return const Color(0xFFA29BFE); // بنفسجي فاتح افتراضي
    }
  }
}
